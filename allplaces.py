...
map= folium.Map(location=[10.796019890314097, 76.82275843702458],zoom_start=16)#opens the map needed by taking the centre point as the given co-ordinates

html= '<img src= "data:image/png;base64,{}">'.format#formating all image files to be added for html

#for location 1: Taste of malabar
picture1= base64.b64encode(open("./tom.jpg",'rb').read()).decode()#encoding and decoding the image file
...
ricon= folium.features.CustomIcon('img.png',icon_size=(40,40))#customized icon
...
folium.Marker(location=[10.795959805319832, 76.82282023370011],tooltip="<strong>Ahalia Sculpture Park</strong>",popup=popup3,icon=ricon3).add_to(map)
...
folium.Marker(location=[10.792262269988168, 76.82803937041948],tooltip="<strong>Ahalia Food Point</strong>",popup=popup5,icon=ricon5).add_to(map)
...
folium.Marker(location=[10.792077430340598, 76.82749318189833],tooltip="<strong>Ahalia Diabetic Hospital</strong>",popup=popup7,icon=ricon7).add_to(map) 
...
folium.Marker(location=[10.791929710674356, 76.82899214327897],tooltip="<strong>Ahalia Ayurveda Hospital</strong>",popup=popup8,icon=ricon8).add_to(map) 
...
folium.Marker(location=[10.799205621030923, 76.82223021773736],tooltip="<strong>Hostel 1</strong>",popup=popup_h1,icon=ricon_h1).add_to(map)
...
folium.Marker(location=[10.799550455187008, 76.8222669062808],tooltip="<strong>Hostel 2</strong>",popup=popup_h2,icon=ricon_h2).add_to(map)
...
folium.Marker(location=[10.797984370712905, 76.81832584827522],tooltip="<strong>Hostel 3</strong>",popup=popup_h3,icon=ricon).add_to(map)
...
folium.Marker(location=[10.794969103780806, 76.82528481763399],tooltip="<strong>Lake park</strong>",popup=popup_l,icon=ricon_l).add_to(map)

#for location 14: Manju's Canteen
picture_man= base64.b64encode(open('stone.png','rb').read()).decode()#change
iframe_man= IFrame(html(picture_man),width=327+20,height=160+20)
popup_man= folium.Popup(iframe_man,max_width=650)
ricon_man= folium.features.CustomIcon('shop.png',icon_size=(60,60))
folium.Marker(location=[10.801940636611368, 76.81835829549671],tooltip="<strong>Manju's Canteen</strong>",popup=popup_man,icon=ricon_man).add_to(map)
...
folium.Marker(location=[10.799290457287118, 76.82241465360843],tooltip="<strong>Mess</strong>",popup=popup_mess,icon=ricon_mess).add_to(map)
...
folium.Marker(location=[10.800699183782722, 76.81830260137629],tooltip="<strong>Pharmacy Block</strong>",popup=popup_ph,icon=ricon_ph).add_to(map)
...
folium.Marker(location=[10.800427387710556, 76.81835704549528],tooltip="<strong>Canara Bank ATM</strong>",popup=popup_ac,icon=ricon_ac).add_to(map)

#for location 19: Basket Ball court
picture_bk= base64.b64encode(open('acad.jpg','rb').read()).decode()#change
...
picture_st= base64.b64encode(open('afp1.jpg','rb').read()).decode()#change
iframe_st= IFrame(html(picture_st),width=327+20,height=160+20)
popup_st= folium.Popup(iframe_st,max_width=650)
ricon_st= folium.features.CustomIcon('home.png',icon_size=(60,60))
folium.Marker(location=[10.797483655650366, 76.81746333689723],tooltip="<strong>Staff Quaters</strong>",popup=popup_st,icon=ricon_st).add_to(map)

#for location 21: Statue
picture_stat= base64.b64encode(open("cdc.jpg",'rb').read()).decode()#change
...
folium.Marker(location=[10.79439732616303, 76.82345431105006],tooltip="<strong>Ahalia Statue</strong>",popup=popup_stat,icon=ricon_stat).add_to(map)
...
folium.Marker(location=[10.799334989848212, 76.82103835492995],tooltip="<strong>MBA Block</strong>",popup=popup_mba,icon=ricon_mba).add_to(map)
